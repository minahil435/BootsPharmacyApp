...
class MainViewController: UIViewController {
    ...
    func setupView(_ viewName : MainStruct?){
        if let _viewName = viewName{
            ...
            if let _newView = newView as? postalCode {
                _newView.delegate = self
            }
            if let _newView = newView as? SelectAddress {
                _newView.delegate = self
            }
...
extension MainViewController : PostalCodeCallDelegate{
    func PostalCodeCallFailure() {
        let alert = UIAlertController(title: "Alert", message: "PostalCode is invalid", preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "Done", style: UIAlertAction.Style.default, handler: nil))
        self.present(alert, animated: true, completion: nil)
    }
}

extension MainViewController : SelectAddressDelegate{
    func SelectAddressVC() {
        let nextViewController = self.storyboard?.instantiateViewController(withIdentifier: "SelectAddressViewController") as! SelectAddressViewController
